...
public class AerodynamicCalculator : SerialReceive
{
    ...
    public void InputSpecifications()
    {
        ...
        else if (GameManager.instance.PlaneName == "Tatsumi")
        {
            ...
            PlaneRigidbody.centerOfMass = new Vector3(0f, 0.053f, 0f);
            PlaneRigidbody.inertiaTensor = new Vector3(993.022f, 1028.254f, 50.789f);
            PlaneRigidbody.inertiaTensorRotation = Quaternion.AngleAxis(-5.102f, Vector3.forward);
            // Specification At Cruise without Ground Effect
            Airspeed0 = 10.500f; // Magnitude of ground speed [m/s]
            alpha0 = 0.000f; // Angle of attack [deg]
            CDp0 = 0.015f; // Parasitic drag [-]
            ...
            CLMAX = 1.400f;
            // Wing
            Sw = 17.92f; // Wing area of wing [m^2]
            bw = 24.514f; // Wing span [m]
            cMAC = 0.812f; // Mean aerodynamic chord [m]
            aw = 0.103f; // Wing Lift Slope [1/deg]
            ...
            // Tail
            Downwash = true; // Conventional Tail: True, T-Tail: False
            St = 1.736f; // Wing area of tail
            at = 0.079f; // Tail Lift Slope [1/deg]
            lt = 2.900f; // Length between Tail a.c. and c.g.
            deMAX = 10.000f; // Maximum elevator angle
            tau = 1.000f; // Control surface angle of attack effectiveness [-]
            VH = (St * lt) / (Sw * cMAC); // Tail Volume
            ...
            Cyp = -0.541162f; // [1/rad]
            Cyr = 0.170132f; // [1/rad]
            Cydr = 0.002134f; // [1/deg]
            Clb = -0.004723f; // [1/deg]
            Clp = -0.794553f; // [1/rad]
            Clr = 0.20325f; // [1/rad]
            Cldr = 0.000044f; // [1/deg]
            Cnb = -0.000285f; // [1/deg]
            Cnp = -0.100557f; // [1/rad]
            Cnr = -0.007178f; // [1/rad]
        ...
        */
        ...
        using (CsvIO csv = new CsvIO(recordCount, fieldCount))
        {
            ...
            try
            {
                ...
                for (int i = 1; i < recordCount; i++) // `csv.Read()`は(1, 1)から開始する.
                {
                    for (int j = 1; j < fieldCount; j++)
                    {
                        ...
                        switch (value)
                        {
                            // 機体の重量と慣性モーメント.
                            case "mass":
                                PlaneRigidbody.mass = float.Parse(csv.Read(i, j + 1));
                ...
                // 機体の重量と慣性モーメント.
                Debug.Log("mass: " + PlaneRigidbody.mass);
                Debug.Log("centerOfMass: " + PlaneRigidbody.centerOfMass);
                Debug.Log("inertiaTensor: " + PlaneRigidbody.inertiaTensor);
                Debug.Log("inertiaTensorRotation: " + PlaneRigidbody.inertiaTensorRotation);
